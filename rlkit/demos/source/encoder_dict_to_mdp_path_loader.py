    ...
    def preprocess(self, observation, use_latents=True):
        ...
        gripper_states = np.stack([
            np.concatenate(
                (observation[i]['state_observation'][:3],
                 quat_to_deg(observation[i]['state_observation'][3:7]) / 360.),
                axis=0)
            for i in range(len(observation))
        ])
        ...
        for i in range(len(observation)):
            ...
            if use_latents:
                ...
                observation[i]['initial_image_observation'] = (
                    observation[0]['image_observation'])
                observation[i]['image_achieved_goal'] = (
                    observation[i]['image_observation'])
                observation[i]['image_desired_goal'] = (
                    observation[-1]['image_observation'])
    ...
    def load_path(self, path, replay_buffer, obs_dict=None, use_latents=True):
        ...
        if obs_dict:
            traj_obs = self.preprocess(
                path['observations'], use_latents=use_latents)
            next_traj_obs = self.preprocess(
                path['next_observations'], use_latents=use_latents)
        else:
