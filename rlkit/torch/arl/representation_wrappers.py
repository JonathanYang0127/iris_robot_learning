...
class RepresentationWrappedEnv(ProxyEnv):
    ...
    def reset(self):
        ...
        # self.set_goal(goal)
        self._initial_obs = self._update_obs(obs)
        return self._initial_obs

    def step(self, action):
        obs, reward, done, info = self.wrapped_env.step(action)
        new_obs = self._update_obs(obs)
        latent = new_obs['latent_observation']
        self._update_info(info, new_obs)
        reward = self.compute_reward(
            action,
            {'observation': latent,
             'latent_achieved_goal': latent,
             'latent_desired_goal': latent}
        )
    ...
    def _encode(self, imgs):
        output = self.vae(ptu.from_numpy(imgs))
        return ptu.get_numpy(output)
    ...
    def compute_rewards(self, actions, obs):
        xt = obs["observation"]
        x0 = self._initial_obs["observation"]
        return np.linalg.norm(xt - x0, ord=self.norm_order, axis=1)
        # return 0
