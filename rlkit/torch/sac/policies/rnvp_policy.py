...
class ObservationConditionedRealNVP(nn.Module, ExplorationPolicy):
    """
    ...
    def get_action(self, obs_np, x=None):
        if x is None:
            x = ptu.from_numpy(np.random.normal(size=(1, self.action_dim)))
        else:
            x = ptu.from_numpy(np.expand_dims(x, axis=0))

        if isinstance(obs_np, dict):
            obs = {}
            for k in obs_np.keys():
                obs[k] = ptu.from_numpy(np.expand_dims(obs_np[k], axis=0))
        else:
            obs = ptu.from_numpy(np.expand_dims(obs_np, axis=0))
