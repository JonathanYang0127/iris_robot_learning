...
class CLearningTrainer(TorchTrainer):
    ...
    def test_from_torch(self, batch):
        ...
        random_obs = batch['random_obs']
        future_obs = batch['future_obs']

        obs = torch.cat([states, goals], dim=1)
        next_obs = torch.cat([next_states, goals], dim=1)
    ...
    def train_from_torch(self, batch, train=True, pretrain=False,):
        ...
        # Temporal Difference C-learning (Alg. 3, Eq. 8) pseudocode
        def classifier_loss_td(C, s_current, a_current, s_next, a_next, s_random, s_future, gamma):
            obj_a = (1 - gamma) * torch.log(C(s_current, s_next, a_current, ))
