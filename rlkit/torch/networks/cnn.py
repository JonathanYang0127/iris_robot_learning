...
class TwoChannelCNN(PyTorchModule):
    ...
    def __init__(
        ...
        self.conv_norm_layers1 = nn.ModuleList()
        self.pool_layers1 = nn.ModuleList()
        ...
        for i, (out_channels, kernel_size, stride, padding) in enumerate(
            ...
            conv1 = nn.Conv2d(input_channels,
                             ...
                             stride=stride,
                             padding=padding)
            hidden_init(conv1.weight)
            conv1.bias.data.fill_(0)
            conv_layer1 = conv1
            self.conv_layers1.append(conv_layer1)
        ...
        for i, conv_layer in enumerate(self.conv_layers):
            ...
            if self.conv_normalization_type == 'batch':
                self.conv_norm_layers.append(nn.BatchNorm2d(test_mat.shape[1]))
                self.conv_norm_layers1.append(nn.BatchNorm2d(test_mat1.shape[1]))
            if self.conv_normalization_type == 'layer':
                self.conv_norm_layers.append(nn.LayerNorm(test_mat.shape[1:]))
                self.conv_norm_layers1.append(nn.LayerNorm(test_mat1.shape[1:]))
            if self.pool_type != 'none':
                test_mat = self.pool_layers[i](test_mat)
    ...
    def forward(self, input, return_last_activations=False):
        ...
        h1 = self.apply_forward_conv1(h1)
    ...
    def apply_forward_conv1(self, h):
        for i, layer in enumerate(self.conv_layers1):
            h = layer(h)
            if self.conv_normalization_type != 'none':
                h = self.conv_norm_layers1[i](h)
            if self.pool_type != 'none':
                h = self.pool_layers1[i](h)
            h = self.hidden_activation(h)
        return h
...
class ConcatTwoChannelCNN(TwoChannelCNN):
    """
    Concatenate inputs along dimension and then pass through MLP.
    """
    def __init__(self, *args, dim=1, **kwargs):
