...
class GatedPixelCNN(nn.Module):
    ...
    def forward(self, x, cond=None):
        ...
        if cond is None:
            cond = torch.zeros(1, dtype=torch.long, device=param.device)

        # print(param.device)
        # print(x.get_device())
        # print(cond.get_device())

        x_v, x_h = (x, x)
        for i, layer in enumerate(self.layers):
            x_v, x_h = layer(x_v, x_h, cond)

        return self.output_conv(x_h)

    def generate(self, shape=None, batch_size=None, cond=None):

        if cond is not None:
            assert shape is None
            assert batch_size is None
            shape = (cond.shape[-2], cond.shape[-1])
        ...
        x = torch.zeros(
            (batch_size, *shape),
            dtype=torch.long, device=param.device
        )

        for i in range(shape[0]):
            for j in range(shape[1]):
                # might need to convert 0 to long
