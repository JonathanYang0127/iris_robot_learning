...
class AffordanceTrainer(LossFunction):
    ...
    def train_batch(self, epoch, batch, should_log, should_train_classifier):
        ...
        if not self.use_pretrained_vqvae:
            raise NotImplementedError('Please use pretrained VQVAE.')
    ...
    def _plot_images(self, plot_data, epoch, prefix):
        ...
        # s_pred = []
        # for h_pred_t in h_pred:
        #     s_pred_t = self.vqvae.decode(h_pred_t, self.prediction_mode)
        #     s_pred.append(s_pred_t)

        s_pred = torch.unbind(s_pred, axis=1)
        ...
        s0 = torch.stack(s0, axis=0)

        h0 = plot_data['h'][:, 0]
        h0 = [h0] * self.num_goal_samples
        h0 = torch.stack(h0, axis=0)
