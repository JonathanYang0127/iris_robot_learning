import os
import numpy as np
import argparse
import time
import pickle

from rlkit.data_management.multitask_replay_buffer import ObsDictMultiTaskReplayBuffer
import rlkit.torch.pytorch_util as ptu

import torch.nn as nn
from torch import optim

from rlkit.launchers.config import LOCAL_LOG_DIR
from rlkit.core import logger
from rlkit.launchers.launcher_util import setup_logger

from rlkit.misc.wx250_utils import (add_multitask_data_to_singletask_buffer_real_robot,
    add_multitask_data_to_multitask_buffer_real_robot, DummyEnv,
    configure_dataloader_params)
from rlkit.torch.task_encoders.encoder_decoder_nets import TransformerEncoderDecoderNet
from rlkit.torch.task_encoders.transformer_action_decoder_trainer import TransformerTrajectoryEncoderTrainer
import argparse


def enable_gpus(gpu_str):
    if gpu_str != "":
        os.environ["CUDA_VISIBLE_DEVICES"] = gpu_str
    return


def main(args):
    buffers = set()
    for buffer_path in args.buffers:
        if '.pkl' in buffer_path or '.npy' in buffer_path:
            buffers.add(buffer_path)
        else:
            path = Path(buffer_path)
            buffers.update(list(path.rglob('combined_trajectories.npy')))
    buffers = [str(b) for b in buffers]
    num_tasks = len(buffers)

    variant = dict(
        buffers=buffers,
        beta_target=args.beta_target,
        beta_anneal_steps=args.beta_anneal_steps,
        latent_dim=args.latent_dim,
        total_steps=int(5e5),
        batch_size=args.batch_size,
        num_tasks=num_tasks,
        image_augmentation=args.use_image_aug,
        path_len=110,
        action_dim=7,
        dataloader_params=dict(
            stack_frames=False,
            downsample_image=True,
            mixup=False,
            action_relabelling='none',
            align_actions=False
        )
    )
    configure_dataloader_params(variant)

    enable_gpus(args.gpu)
    ptu.set_gpu_mode(True)


    max_replay_buffer_size = int(1e6)
    image_size = 64
    expl_env = DummyEnv(image_size=image_size)
    train_task_indices = list(range(variant['num_tasks']))
    observation_keys = ['image',]

    replay_buffer = ObsDictMultiTaskReplayBuffer(
        max_replay_buffer_size,
        expl_env,
        train_task_indices,
        use_next_obs_in_context=False,
        sparse_rewards=False,
        observation_keys=observation_keys,
        path_len=variant['path_len']
    )
	
    replay_buffer_val = ObsDictMultiTaskReplayBuffer(
        int(max_replay_buffer_size/2),
    	expl_env,	
        train_task_indices,
	use_next_obs_in_context=False,
	sparse_rewards=False,
	observation_keys=observation_keys,
        path_len=variant['path_len']
    )
    
    buffer_params = {task: b for task, b in enumerate(variant['buffers'])}
    add_multitask_data_to_multitask_buffer_real_robot(buffer_params, replay_buffer,
	    multitask_validation_buffer=replay_buffer_val,
	    validation_fraction=0.2) 
    print(replay_buffer.task_buffers[0].max_path_length, variant['path_len'])

    latent_dim = variant['latent_dim']
    net = TransformerEncoderDecoderNet(image_size, latent_dim, variant['path_len'],
        image_augmentation=args.use_image_aug,
        encoder_keys=['observations'],
        decoder_output_dim=variant['action_dim'],
        decoder_type='medium'
        )
    net.to(ptu.device)
    exp_prefix = '{}-task-encoder-decoder'.format(time.strftime("%y-%m-%d"))
    save_freq = 100
    setup_logger(logger, exp_prefix, LOCAL_LOG_DIR, variant=variant,
                 snapshot_mode='gap_and_last', snapshot_gap=save_freq, )


    batch_size = variant['batch_size']

    optimizer = optim.Adam(net.parameters(), lr=3e-4)
    # log_alpha = ptu.zeros(1, requires_grad=True)
    # alpha_optimizer = optim.Adam([log_alpha], lr=0.01)

    print_freq = 100
    total_steps = variant['total_steps']
    half_beta_target_steps = min(total_steps // 2, args.beta_anneal_steps)
    beta_target = args.beta_target
    criterion = nn.MSELoss()
    tasks_to_sample = list(range(variant['num_tasks']))

    trainer = TransformerTrajectoryEncoderTrainer(net, optimizer, criterion, print_freq, save_freq,
                                 beta_target, half_beta_target_steps, args.anneal)

    trainer.train(replay_buffer, replay_buffer_val,
                 total_steps, batch_size, tasks_to_sample, logger)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--buffers", nargs='+', required=True)
    parser.add_argument("--anneal", type=str, default='linear',
                        choices=('sigmoid', 'linear', 'none'))
    parser.add_argument("--latent-dim", default=2, type=int)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--gpu", default='0', type=str)
    parser.add_argument("--beta-target", type=float, default=0.001)
    parser.add_argument("--beta-anneal-steps", type=int, default=10000)
    parser.add_argument("--use-image-aug", default=False, action='store_true')
    parser.add_argument("--use-alpha", default=False, action='store_true')
    args = parser.parse_args()
    main(args)
