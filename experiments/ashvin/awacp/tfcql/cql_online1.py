...
def train_eval(
  ...
  root_dir = "%s/%s/%s/%d/" % (root_dir, exp_name, env_name, seed)
  ...
  expl_greedy_policy = py_tf_eager_policy.PyTFEagerPolicy(
      agent.policy, use_tf_function=True)
  expl_actor = actor.Actor(
      env,
      expl_greedy_policy,
      train_step,
      observers=replay_observers,
      metrics=actor.eval_metrics(eval_episodes),
      summary_dir=os.path.join(root_dir, 'expl'),
      episodes_per_run=eval_episodes)
