...
if __name__ == "__main__":
    ...
    search_space = {
        ...
        'grill_variant.online_vae_trainer_kwargs.beta': [
            30,
        ],
    }
    ...
    n_seeds = 3
    mode = 'sss'
    exp_name = 'one-obj-pygame-pnp-skew-fit-proper-sweep'

    for exp_id, variant in enumerate(sweeper.iterate_hyperparameters()):
        variant['train_vae_variant']['algo_kwargs']['skew_config']['power'] = (
            variant['grill_variant']['replay_buffer_kwargs']['power']
        )
        if mode == 'local':
            variant['grill_variant']['algo_kwargs'] = dict(
                batch_size=8,
                num_epochs=5,
