...
if __name__ == "__main__":
    ...
    search_space = {
        ...
        'dynamics_delta_model_config.log_std_min': [
            float(np.log(0.1)),
            float(np.log(0.01)),
            float(np.log(0.001)),
        ],
    ...
    n_seeds = 2
    mode = 'ec2'
    exp_name = 'pgr--partial-goal-model--exp-7-fetch-push-auto-reward-scale-model-with-std-floor-take2'
    ...
    for exp_id, variant in enumerate(sweeper.iterate_hyperparameters()):
        ...
        if (variant['reward_type'] == 'sparse'
            and variant['dynamics_model_version'] == 'learned_model'):
            continue
