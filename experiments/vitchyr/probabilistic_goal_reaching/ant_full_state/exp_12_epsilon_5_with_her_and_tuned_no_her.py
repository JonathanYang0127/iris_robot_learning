...
if __name__ == "__main__":
    ...
    n_seeds = 4
    mode = 'htp'
    exp_name = 'pgr--ant-full-state--exp-9--epsilon-5-with-her-and-tuned-no-her'
    ...
    for exp_id, variant in enumerate(sweeper.iterate_hyperparameters()):
        if (
                variant['success_threshold'] == 5.0
            and variant['replay_buffer_kwargs']['fraction_future_context'] != 0.8
        ):
            continue
        if (
                variant['success_threshold'] == 1.0
                and variant['replay_buffer_kwargs']['fraction_future_context'] != 0.
        ):
            continue
