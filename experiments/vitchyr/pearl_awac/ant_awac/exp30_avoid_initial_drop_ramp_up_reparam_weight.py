...
def main(debug, dry, suffix, nseeds):
    ...
    def run_sweep(search_space, variant, xid):
        for k, v in {
            ...
            'online_trainer_kwargs.reparam_weight_schedule_kwargs': [
                dict(
                    version='piecewise_linear',
                    x_values=[0, int(1e5), int(2e5), int(3e5), int(4e5), int(5e5)],
                    y_values=[0., 0., 0., 1, 1.0, 1.0],
                ),
                dict(
                    version='piecewise_linear',
                    x_values=[0, int(1e5), int(2e5), int(3e5), int(4e5), int(5e5)],
                    y_values=[0., 0., 0.01, 0.1, 1.0, 1.0],
                ),
            ],
